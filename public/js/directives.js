  ...
  .directive('player', () => {
    ...
    .players[scope.game.winningCardPlayer].color]}>`;
            ...
            const removePunctuation = (cardIndex) => {
              ...
              if (cardText
                .indexOf('.', cardText.length - 2) === cardText.length - 1) {
            ...
            if (curQuestionArr.length > 1) {
              ...
              if (shouldRemoveQuestionPunctuation) {
                if (curQ.text
                  .indexOf('.', curQ.text.length - 2) === curQ
                    .text.length - 1) {
                  curQ.text = curQ.text.slice(0, curQ.text.length - 2);
                }
              }
            } else {
              curQ.text += `${startStyle} ${scope.game
                .table[scope.game.winningCard].card[0].text} ${endStyle}`;
            }
          }
        });
      }
